"""
Stage 3: NVMe sorted/ → 카메라 기종별 HDD 배포 (새벽 6시 실행)
단일 패스: NVMe에서 모든 HDD에 직접 복사 → 모든 HDD 성공 시 NVMe 원본 삭제.
SHA-256 검증은 매주 일요일 새벽 checker.py에서 일괄 수행.
"""
import hashlib
import json
import os
...
def _copy_to_hdd(src: str, hdd_root: str, rel_path: str, manifest: dict,
    ...
    SHA-256 검증은 일요일 무결성 검사(checker.py)에서 수행.
    """
...
def distribute():
    """sorted/ 내 파일을 NVMe에서 모든 HDD로 단일 패스 배포."""
    ...
    for h in sorted(manifests):
        if os.path.isdir(h):
            free = shutil.disk_usage(h).free / 1024**3
            lines.append(f"{os.path.basename(h)}: {free:.1f} GB 남음")

    print(f"\n[배포 완료] {saved_count}장" + (f"  ({date_str})" if date_str else ""))
    if errors:
        for e in errors[:3]:
            print(f"  [오류] {e}")

    notify(title, "\n".join(lines), priority=priority, tags=[tag])
